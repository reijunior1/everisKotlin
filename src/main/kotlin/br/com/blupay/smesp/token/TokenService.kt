...
class TokenService(
    ...
    fun moveToken(token: String, signer: WalletTemp, debtorId: UUID, creditors: List<Creditor>): Mono<MoveTokensRequest> {
        return nodeProvider.getPublicKey(token).map { node ->
            MoveTokensRequest(
                    TokenHeader(
    ...
    fun safeMoveToken(token: String, signer: WalletTemp, dueDate: Long, debtorId: UUID, ioyWalletId: UUID, creditors: List<Creditor>): Mono<SafeMoveTokensRequest> {
        return nodeProvider.getPublicKey(token).map { node ->
    ...
    fun settlementOrderToken(token: String, signer: WalletTemp, dueDate: Long, debtorId: UUID, cashoutWalletId: UUID, creditors: List<Creditor>): Mono<SettlementMoveTokensRequest> {
        return nodeProvider.getPublicKey(token).map { node ->
